...
Func SwitchCOCAcc_ClickAccountSCID(ByRef $bResult, $NextAccount, $iStep = 2, $bVerifyAcc = True, $bDebuglog = $g_bDebugSetlog, $bDebugImageSave = $g_bDebugImageSave)
	...
	Local $aSearchForAccount, $aFound, $aCoordinates[0][2], $aTempArray
	...
	For $i = 0 To 30 ; Checking "New SuperCellID UI" continuously in 30sec
		...
		If IsArray($aSuperCellIDWindowsUI) And UBound($aSuperCellIDWindowsUI, 1) >= 2 Then
			...
			ClickDrag(540, 400, 540, 375) ; This Will Make It Drag To Show The 4th Account.
			...
			If _Sleep(1000) Then Return
			$aSearchForAccount = QuickMIS("CX", $g_sImgSupercellIDSlots, 790, 320, 840, 732, True, False) ;decodeMultipleCoords(findImage("Account Locations", $g_sImgSupercellIDSlots, $sAccountDiamond, 0, True, Default))
			...
			If IsArray($aSearchForAccount) And UBound($aSearchForAccount) > 0 Then
				For $j = 0 To UBound($aSearchForAccount) - 1
					$aFound = StringSplit($aSearchForAccount[$j], ",", $STR_NOCOUNT)
					_ArrayAdd($aCoordinates, $aFound[0]+750 & "|" & $aFound[1]+320)
			    Next
			    ...
			    If $NextAccount >= 3 Then
					SCIDragIfNeeded($NextAccount)
				    $iIndexSCID = 3 ; based on drag logic, the account will always be the bottom one
			    EndIf
			    ...
			    If UBound($aCoordinates) < 4 And $NextAccount >= 3 Then
				    SetLog("Only Found " & UBound($aCoordinates) & " SCID Account, Select Last Account", $COLOR_INFO)
					$iIndexSCID = UBound($aCoordinates) - 1
			    EndIf
				...
				;If IsArray($aCoordinates) And UBound($aCoordinates) > 0 And UBound($aCoordinates, $UBOUND_COLUMNS) > 0 Then
					...
					Click($aCoordinates[$iIndexSCID][0]-75, $aCoordinates[$iIndexSCID][1] + 10 , 1)
...
Func SCIDragIfNeeded($NextAccount) ;($iSCIDAccount, $bVerifyAcc = True)
	If Not $g_bRunState Then Return
	...
	For $i = 0 To $NextAccount - 4
		If Not $g_bRunState Then Return
		ClickDrag(590, 500, 590, 388)
	Next
